...
/*
      ...
      classMap = map.getClassMap(action.getElementTypeName());
      if (type == PropertyMapBase.ELEMENTTYPE)
      {
         propMap = classMap.getChildMap(propName);
      }
      else if (type == PropertyMapBase.ATTRIBUTE)
      {
         propMap = classMap.getAttributeMap(propName);
      }
      else // if (type == PropertyMapBase.PCDATA)
      {
         propMap = classMap.getPCDATAMap();
      }
      column = propMap.getColumn();

      // Check that the property column is not in the primary key, a key generated
      // by a key generator, or a key generated by the database.

      table = classMap.getTable();
      columns = table.getPrimaryKey().getColumns();
      if (columns.contains(column))
         throw new XMLMiddlewareException("Update property " + propName.getUniversalName() + " is mapped to the " + column.getName() + " in the primary key. Primary key columns may not be updated.");

      columns = table.getKeyGeneratorColumns()
      if (columns.contains(column))
         throw new XMLMiddlewareException("Update property " + propName.getUniversalName() + " is mapped to the " + column.getName() + ", which is generated by a key generator. Columns generated by key generators may not be updated.");

      columns = table.getDBGeneratedColumns()
      if (columns.contains(column))
         throw new XMLMiddlewareException("Update property " + propName.getUniversalName() + " is mapped to the " + column.getName() + ", which is generated by the database. Columns generated by the database may not be updated.");

      // Set the update property on the action.
*/
